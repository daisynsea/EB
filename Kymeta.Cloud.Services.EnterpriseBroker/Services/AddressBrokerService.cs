    ...
    public async Task<UnifiedResponse> ProcessAddressAction(SalesforceAddressModel model)
    {
        ...
        */
        ...
        if (syncToOracle)
        {
            ...
            var organizationResult = await _oracleService.GetOrganizationById(model.ParentAccountId, salesforceTransaction, model.ParentOracleAccountPartyId);
            ...
            var addressIds = new List<Tuple<string, string, string>> { 
                new Tuple<string, string, string>(model.ObjectId, model.OracleLocationId, model.OraclePartySiteId) 
            };
            ...
            else
            {
                ...
                var orgPartySite = organization.PartySites?.FirstOrDefault(ps => ps.OrigSystemReference == model.ObjectId || ps.PartySiteId.ToString() == model.OraclePartySiteId);
            ...
            var accountSite = customerAccount.Sites?.FirstOrDefault(s => s.OrigSystemReference == model.ObjectId || s.PartySiteId.ToString() == model.OraclePartySiteId);
            if (accountSite == null)
            {
